...
- [?] Regenerate the ignored digest and manifest, verify matching hash, `dirty: false`, no recursive third-party file bodies, and clean Git status. Hash matching and third-party exclusion pass; clean-snapshot verification awaits a clean working tree because pre-existing user work is present.
- [x] Update documentation if the temporary-artifact behavior needs operator-facing explanation.
- [ ] Archive the completed plan, refresh indexes, journal the correction, and commit locally without pushing.
