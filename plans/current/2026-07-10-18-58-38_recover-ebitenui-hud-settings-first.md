...
- [?] 3. Restore the Settings tab and update-button path first.
  ...
  - [x] 3.8 Recreate the Settings checkbox/toggle that opens the floating draggable Debug UI overlay.
  - [x] 3.9 Populate the Debug UI overlay with at least screen size, FPS, UPS, working/runtime path, binary path, active tab, input route, and recent diagnostic state.
...
- [?] 4. Rebuild non-Settings tab bodies under the EbitenUI paradigm.
  - [x] 4.1 Define the restored EbitenUI tab-body patterns for Settings fieldsets, Research placeholder/review content, and master-detail tabs.
  - [x] 4.2 Define the responsive master-detail breakpoint where two-column bodies collapse to single-pane mode.
  - [x] 4.3 In collapsed mode, show the left/list pane first; selecting a thread, row, or section switches to the right/detail pane.
  - [x] 4.4 In collapsed detail mode, render an EbitenUI `<- Back` button at the top that returns to the left/list pane.
  - [x] 4.5 In expanded two-column mode, add a visible draggable divider between left and right panes and persist or reset its width according to the approved HUD state policy.
  - [x] 4.6 Restore Comrades from `internal/hud` sections with responsive master-detail content.
  - [x] 4.7 Restore Projects from `internal/hud` sections with responsive master-detail content.
  - [x] 4.8 Restore Research as a readable placeholder/review body that follows the same responsive pattern when it has list/detail structure.
  - [x] 4.9 Restore Cluster from `internal/hud` sections with responsive master-detail content.
  - [x] 4.10 Restore Routing from `internal/hud` sections with responsive master-detail content.
  - [x] 4.11 Restore Tasks from `internal/hud` sections with responsive master-detail content.
  - [ ] 4.12 Add visual validation evidence for every restored tab or explicitly mark any tab as still blocked.
  - [x] 4.13 Make the check for updates always happen automatically in the background when the app opens, but silently unless there is an update available.
...
- [?] 5. Reintegrate or replace expected HUD behavior.
  ...
  - [x] 5.3 Restore horizontally scrollable/swipeable top tab buttons so small screens can pan through hidden tabs and tap the desired tab.
  - [x] 5.4 Ensure keyboard/controller tab changes scroll the tab strip enough to keep the active tab visible.
  - [x] 5.5 Preserve 44px minimum touch targets for tabs, list rows, divider handles, Back buttons, toggles, and action buttons.
  - [x] 5.6 Keep diagnostics and the Debug UI overlay visible only when requested, without covering primary Settings/update controls by default.
...
- [?] 6. Sweep stale custom-layout code, tests, and docs.
  ...
  - [x] 6.6 Update `playbooks/how_to_add_or_modify_hud_tab_contents.md` if the practical EbitenUI recovery reveals better constraints.
...
- 2026-07-10: Implemented the remaining EbitenUI recovery behavior in commits `4e5ec72` and `7d31e78`: replaced the top-level `TabBook` with a scrollable EbitenUI tab-button strip, added responsive master-detail collapse with `<- Back`, added a visible draggable divider in expanded mode, restored a Settings-controlled draggable Debug UI overlay, updated GUI/playbook docs, and rebuilt Linux and Android artifacts. `go test -c -tags gui`, `make test`, `make check-docs`, code-size checks, `git diff --check`, `make build`, and `make build-android` pass. Running GUI-tagged tests and capturing fresh device screenshots remain blocked by display/ADB approval limits.
- 2026-07-10: Added Android startup update checking through `MainActivity.checkForUpdate(false)`. The startup path is silent when the tracked APK matches the installed APK and only surfaces user-facing update/permission/install states when action is needed; manual Settings checks still report `Checking...`, `Already current`, and failures.
- 2026-07-10: Final local verification after the startup update-check change passed `go test -c -tags gui -o /tmp/apparat-gui.test ./internal/adapters/gui`, `make test`, `make check-docs`, `python3 scripts/check_code_file_lines.py`, `git diff --check`, `make build`, and `make build-android`. Linux and Android artifacts were rebuilt. Device install/screenshot validation still needs an ADB approval window.
