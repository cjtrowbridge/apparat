...
- [x] 3. Fix broken Unicode tab labels.
  - [x] 3.1 Revert `ui_builder.go` `buildTabs` logic to strictly use `tabData.Title()`.
  - [x] 3.2 Drop the unsupported `tabData.Descriptor.Glyph` from the layout configuration.
...
**Execution Tasks:**
- [x] 4. Introduce bordered graphics.
  - [x] 4.1 Introduce `createBorderedNineSlice` in `theme.go`.
    - [x] 4.1.1 Programmatically create a 3x3 graphic using `bodyBorderColor` for the outer edge and a distinct fill color for the inner square.
  - [x] 4.2 Apply this bordered NineSlice to `theme.PanelTheme` and `ScrollContainerImage` so layout panes have crisp borders.
...
2. It is hardcoded at the top of the settings page rather than correctly mapping to the `settings.updates.check_for_update` row ID from the snapshot data.
...
- **Dynamic Placement:** Inside the new dynamic `buildSettingsTab` loop, when encountering a row with ID `settings.updates.check_for_update`, render this `widget.Button` instead of a plain text row.
...
- [x] 5. Fix update button logic and aesthetics.
  - [x] 5.1 Apply the bordered NineSlice to `theme.ButtonTheme` (with a contrasting fill like `accentColor` or `fieldsetColor`) so buttons look like physical buttons.
  - [x] 5.2 Map the "Check for update" button to the correct dynamic row ID.
    - [x] 5.2.1 Add conditional logic in the `section.Rows` loop to intercept `settings.updates.check_for_update`.
    - [x] 5.2.2 Create the interactive EbitenUI update button at that specific row instead of a plain text row.
...
- [x] 6. Update playbooks and documentation.
  - [x] 6.1 Edit `playbooks/how_to_add_or_modify_hud_tab_contents.md`.
    ...
    - [x] 6.1.2 Document the requirement to use bordered NineSlice graphics for buttons so they are visually distinct.
