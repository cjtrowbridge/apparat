...
Comrades, Projects, Cluster, and Tasks render as responsive master-detail bodies. Expanded layouts show the left list, a visible draggable divider, and only the selected right detail pane. Narrow layouts collapse to one pane: list first, selected section/thread switches to detail, and detail starts with an EbitenUI `<- Back` button. Cluster's Routing selector and Projects' Pipelines selector render grouped nested detail sections in that same right pane. Research uses the pattern that matches its current placeholder/review state. Future scrolling, tab overflow behavior, narrow-screen collapse, and adjustable dividers must preserve the same minimum-width and no-overlap rules, and must be validated with screenshots before release.
