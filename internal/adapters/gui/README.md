...
`selector_panel_test.go` proves Cluster selector headings remain non-selectable text and that its Tasks selector owns the Tasks content panel. Keep it focused on selector/content-panel semantics; ordinary shell coverage remains in `ebiten_shell_test.go`.

`table.go` owns the reusable `borderedTable` routine for HUD data tables. Give it one header `tableRow` and same-width data rows; it applies one shared grid, a table perimeter, and a border around every header/data cell. Use `tableText` for padded plain-text cells. Do not recreate tables as independent header and row grids: each grid calculates columns independently and will misalign. Interactive header widgets remain ordinary EbitenUI controls supplied to the table routine.
...
Settings renders each `internal/hud` section as a fieldset. The `Updates` section owns an EbitenUI-rendered `Check for update` button plus a wrapped status explanation. `update_status.go` maps stable update reason codes to safe short labels and actionable details; render only those mappings, never raw exceptions, response bodies, tokens, private paths, or unverified release data. Android reports codes through the mobile bridge. Desktop has no checker yet, so it truthfully reports `Unable to check for updates` with a no-checker explanation. Production signed manifests, desktop update checks, and remote node updates remain later roadmap work. Do not add a native Android overlay button for this path.
...
Comrades, Projects, and Cluster render responsive selector/content-panel bodies. Expanded layouts show a selector panel, a visible draggable divider, and only the selected content panel. The selector panel may include non-selectable, non-focusable headings that group selectable items; a heading may have a small, muted, wrapped description beneath its title. Narrow layouts collapse to one panel: the selector panel appears first, a selected section/thread switches to the content panel, and the content panel starts with an EbitenUI `<- Back` button. Cluster's Routing and Tasks selectors and Projects' Pipelines selector render grouped nested detail sections in that content panel. Research uses the pattern that matches its current placeholder/review state. Its mock contribution fieldset renders `Your Contribution` separately from a two-column friend table; a shared grid aligns all cells, with an outer border and borders around each cell. Both 44 px headers are clickable and select/toggle the numeric view-model sort without parsing displayed units. Future scrolling, tab overflow behavior, narrow-screen collapse, and adjustable dividers must preserve the same minimum-width and no-overlap rules, and must be validated with screenshots before release.

Selector groups use the ordered HUD palette `#0032AB`, `#6028A7`, `#8C159F`, `#AF0093`, `#CB0084`, `#E10072`, `#F10060`, cycling only after the final entry. A group's heading text and every selector button share its palette color: headings use it as text and buttons use it as an outline, while hover and selected states stay visually distinct. Mock chat panels include an editable no-op reply field and Send button. Project and Pipeline panels include presentation-only Git/Chat controls; Pipelines also expose a no-op Run control.

Rows, list items, buttons, and input-like controls must remain large enough for touchscreens. Tab buttons, Back buttons, selectable selector items, and divider handles use the same 54 px touch scale unless a future plan changes the shared constant. Selector headings are text, not controls, and do not need a touch target. Selector items are left-aligned with leading room reserved for later avatars or status glyphs. Keep minimum touch target constants named and covered by tests when changing tab body layout. Text blocks must remain inside their owning EbitenUI container; they must not draw over adjacent fieldsets or panels.
