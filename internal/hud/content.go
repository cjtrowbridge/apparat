...
func projectsTab() Tab {
	return tab(DefaultConfigManager{}.Config(), TabProjects, "Local project workspace preview with mock chats, files, artifacts, and Git state.", []Section{{Title: "Projects", Description: "This will become a project list with selected project context beside it.", Rows: []Row{{Label: "apparat", Detail: "selected local project mock"}, {Label: "offline draft", Detail: "transaction concept only", Future: true}}}, {Title: "Workspace", Description: "Selected project context groups chats, files, artifacts, and safe Git status.", Rows: []Row{{Label: "Chat preview", Detail: "architecture sketch"}, {Label: "Files", Detail: "README.md, ROADMAP.md placeholders"}, {Label: "Artifacts", Detail: "mock transcript"}, {Label: "Git", Detail: "clean placeholder"}}}})
}
...
func clusterTab() Tab {
	return tab(DefaultConfigManager{}.Config(), TabCluster, "Local diagnostics and mock device capability inventory.", []Section{{Title: "Local Runtime", Description: "Runtime diagnostics summarize this device before real cluster enrollment exists.", Rows: []Row{{Label: "Identity", Detail: "classified by doctor"}, {Label: "Runtime root", Detail: "shown in Settings"}, {Label: "last_run.log", Detail: "reset on each start"}}}, {Title: "Mock Devices", Description: "Cluster will become a device list with selected-device context.", Rows: []Row{{Label: "steamdeck", Detail: "GUI console"}, {Label: "worker", Detail: "text_generation, speech_to_text"}, {Label: "workstation", Detail: "image_generation, video_generation, research_boinc"}}}})
}

func routingTab() Tab {
	return tab(DefaultConfigManager{}.Config(), TabRouting, "Typed workload queues, compatibility filters, and fallback routing.", []Section{{Title: "Workload Classes", Description: "Routes are grouped by explicit workload classes rather than one generic compute flag.", Rows: []Row{{Label: "Text generation", Detail: "OpenAI-compatible, Ollama, llama.cpp"}, {Label: "Image generation", Detail: "future adapter contract"}, {Label: "Video generation", Detail: "future long-running adapter"}, {Label: "Speech-to-text", Detail: "whisper.cpp reference"}, {Label: "Text-to-speech", Detail: "service-backed first"}, {Label: "BOINC", Detail: "research compute, not model inference"}}}, {Title: "Queues", Description: "Routing will become a queue list with selected-route context and controls.", Rows: []Row{{Label: "Priority", Detail: "owner work above comrade/research"}, {Label: "Fallback", Detail: "compatible devices only"}}}})
}

func tasksTab() Tab {
	return tab(DefaultConfigManager{}.Config(), TabTasks, "Future schedules, webhooks, event rules, approvals, and run history.", []Section{{Title: "Task Types", Description: "Tasks will become a run list with selected-task history and approval context.", Rows: []Row{{Label: "Scheduled task", Detail: "placeholder", Future: true}, {Label: "Webhook", Detail: "placeholder", Future: true}, {Label: "Signal-driven", Detail: "future adapter", Future: true}, {Label: "Manual approval", Detail: "planned safety gate", Future: true}}}, {Title: "Controls", Description: "Creation controls stay disabled until durable task storage exists.", Rows: []Row{{Label: "Create task", Detail: "disabled until durable task storage exists", Disabled: true, Future: true}}}})
}

func settingsTab(config HUDConfig) Tab {
	return tab(config, TabSettings, "Local configuration defaults, runtime paths, diagnostics, and verification hints.", []Section{{Title: "HUD Configuration", Description: "These defaults are hard-coded now and move to durable settings later.", Rows: []Row{{Label: "Tab placement", Detail: string(config.TabView.Placement)}, {Label: "Theme", Detail: string(config.Display.Theme)}, {Label: "Scale", Detail: "1.0"}, {Label: "Font size", Detail: "18"}, {Label: "Settings persistence", Detail: "hard-coded now, future SQLite user settings", Future: true}}}, {Title: "Bindings", Description: "Controller, keyboard, mouse, and touch bindings dispatch the same HUD actions.", Rows: []Row{{Label: "Previous tab", Detail: "L1, Ctrl+PageUp"}, {Label: "Next tab", Detail: "R1, Ctrl+PageDown"}, {Label: "Direct tabs", Detail: "Alt+1..Alt+7"}, {Label: "Push-to-talk", Detail: "R2, right Ctrl"}}}, {Title: "Diagnostics", Description: "Local checks and logs support troubleshooting without backend services.", Rows: []Row{{Label: "Doctor", Detail: "run --doctor"}, {Label: "Smoke test", Detail: "run --smoke-test"}, {Label: "Logs", Detail: "last_run.log and logs/apparat.jsonl"}, {Label: "Verify", Detail: "make verify && make check-docs"}}}, {Title: "Updates", Description: "Download the latest tracked APK and let Android ask before installing it."}})
}
